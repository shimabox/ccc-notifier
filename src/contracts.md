...
- `src/dashboard.ts`: 既定で **全履歴**を読み込む(`readTurns()` を days なしで)。`--days N` は埋め込み範囲の任意の
  ...
  - 粒度(acn-gran)・選択(acn-sel)・検索(acn-search)・スクロール(acn-scroll)を sessionStorage に保存し、
    自動リロード(meta refresh)を跨いでも維持する。
  - セキュリティ不変条件は維持: 外部通信ゼロ / #acn-data の < を \\u003c にエスケープ / プロンプトは textContent /
...
- `dashboard.ts`: 予算>0 のとき KPI 直下に `#acn-budget` カードを出す。サーバは当月の初期表示を描画し、
