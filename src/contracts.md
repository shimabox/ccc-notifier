...
- sweepは探索・価格/Fxをlock外、対象transcript/rollout単位でcursor再読込からcommitまでlockする。lock timeout対象はcursorを進めず、他の正常対象をcommitした上で未完了message + exit 1。dry-runはlock不要。
...
- `src/dashboard.ts`: 手動 `dashboard` は既定で `report.html` に `Config.dashboard.days` 分(既定30日)を埋め込む。`--all` は `report-all.html` に全履歴、`--days N` は `report.html` に直近N日を埋め込む。不正な `--days` はfallbackせずexit 1とする。`track.ts` の自動再生成は
  ...
  これにより HTML 構築・書き込み・ブラウザ描画の負荷を抑える。履歴ファイルは予算の正確性を保つため
  `readTurns()` で1回だけ全件 read/parse し、埋め込み対象はその配列から filter するため、この部分は O(全履歴)。
...
- `dashboard.ts`: 予算>0 のとき KPI 直下に `#cccn-budget` カードを出す。手動の全履歴版では、
  ...
  その月へ丸める)。自動再生成などの期間限定版では、埋め込み対象外も含む全履歴から集計した**当月**に固定し、
...
## 2026-07-15 追加: sweep --rebuildによる単純な全再生成

- `ccc-notifier sweep --rebuild [--yes|-y]`は、確認後に既存data lockを1回取得し、canonical dashboardを無効化してから`history.jsonl`と`cursors.json`をbackupなしで削除する。その後、Claude main / agentとCodex rolloutの元JSONLをcursorなし・期間制限なし・active guardなしで全走査し、履歴とcursorを再生成する。
- Claude/Codexの終了・再起動、通知停止marker、maintenance modeは使わない。rebuild中に競合したhookは記録・通知できない場合があり、通知は再送しない。lock解放後は追加操作なしで通常動作へ戻り、読み取り後の末尾は後続hook/通常sweepが回収する。同時truncate/rewriteはbest-effortであり完全性を保証しない。
- reset対象はhistory、cursors、canonical dashboardと日次更新stateだけ。rebuild自身はdashboard HTMLを生成せず、`dashboard.autoRegenerate=true`なら次の正常なturn、無効なら手動`dashboard` / `dashboard --all`に委ねる。config、月予算、通知設定、mute、単価/為替cache、Codex hook、Codexサブエージェント利用記録は保持する。単価/為替cacheはresetしないが、通常loaderのオンライン更新は許容する。
- 全履歴はrebuild実行時点の単価表/FXで再計算し、過去額は変わり得る。clear済み履歴/redact済みpromptもsourceにあれば復活する。source消失・移動・破損行は復元できない。Codexサブエージェント利用記録自体は保持するが、再生成した過去turnへの表示joinは失われ得る。
- source単位のhard failureは終了コード1とし、部分生成をrollbackしない。同じ`sweep --rebuild`を再実行すると履歴/cursorを再度resetして最初から再生成する。
- `--rebuild`と`--dry-run`/`--days`/`--include-active`/`--projects`の併用、rebuildなしの`--yes|-y`、未知option、値不足、余分な位置引数はmutation前に終了コード1で拒否する。
