...
- sweepは探索・価格/Fxをlock外で準備し、通常実行は全sourceのreset/再生成を1つのdata lock内で行う。lock timeout時はhistory/cursors/dashboardを変更しない。dry-runはlock不要でsource先頭からread-only previewする。
...
- reset対象はhistory、cursors、canonical dashboardと日次更新stateだけ。sweep自身はdashboard HTMLを生成せず、`dashboard.autoRegenerate=true`なら次の正常turn、無効なら手動`dashboard` / `dashboard --all`に委ねる。config、月予算、通知設定、mute、単価/為替cache、Codex hook、Codexサブエージェント利用記録は保持する。
