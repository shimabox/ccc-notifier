...
| `sweep [--dry-run] [--days N]` | 通常実行は履歴・取り込み位置・自動生成ダッシュボードをbackupなしでresetし、手元のClaude/Codex JSONLから概算を再生成。正常完了時は自動生成が有効なら直近版・全履歴版も即時更新する。`--days N`は期間内だけ、`--dry-run`は何も変更しないsource先頭からのpreview(詳細は [履歴の再生成](docs/sweep.md)) |
...
`dashboard` コマンドで、サマリー(今日 / 今週 / 今月 / 通算)、コスト推移(日/週/月で切替・横スクロールで過去まで)、モデル別/プロジェクト別内訳、検索・行展開できるターン履歴を HTML に書き出してブラウザで開きます。自動生成物は、毎ターン更新する直近版 `~/.ccc-notifier/report.html`(既定30日)と、ローカル日の最初の正常なターンだけ更新する全履歴版 `~/.ccc-notifier/report-all.html` に分かれます。正常な`sweep`完了時は、自動生成が有効なら両方を即時更新します。片方が未生成でも生成方法を示すplaceholderを置くため、ページ内リンクは切れません。履歴・カーソル・生成snapshotは `cache/data.lock/` で直列化されます。期間限定版でも保存済み全履歴の当月分を集計対象から落とさないため、履歴の読み込みと解析自体は全履歴が対象です。生成物は **CSS/JS/SVG をすべてインライン化した完全自己完結・オフライン動作・外部通信ゼロ**のファイルで、ライト/ダーク両対応です。
...
グラフの棒をクリックするとその期間に内訳・履歴が連動し、全履歴版では「通算」、期間限定版では「対象期間合計」で埋め込まれた全期間に戻せます。直近版は応答完了ごと、全履歴版は1日1回(または手動 `dashboard --all`)更新されます。直近版の表示は、設定期間を満たす履歴があれば「直近 N 日版」、履歴のローカル暦日spanがそれより短ければ「履歴 N 日分」と実際の埋め込み範囲を示します。静的HTMLなので日付が変わるだけでは表示は変わらず、次のターンまたは手動生成で追従します。Codex CLI のレコードがあれば **Claude / Codex** を絞り込むソースフィルタも表示されます(詳細は [Codex CLI 対応](docs/codex.md))。検索・行クリックでプロンプト全文を確認できます:
...
- [履歴の再生成 / sweep](docs/sweep.md) — 元JSONLから全期間または指定期間の概算を作り直す
