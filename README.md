...
| `sweep [--dry-run] [--days N]` | 通常実行は履歴・取り込み位置・自動生成ダッシュボードをbackupなしでresetし、手元のClaude/Codex JSONLから概算を再生成。`--days N`は期間内だけ、`--dry-run`は何も変更しないsource先頭からのpreview(詳細は [履歴の再生成](docs/sweep.md)) |
