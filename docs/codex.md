...
`sweep`は既存のコスト履歴と取り込み位置をresetし、Claude Code / Codexの手元に残る元JSONLから概算を再生成します。Codex対応後は`~/.codex/sessions`配下も自動的に走査対象になります(`~/.codex`が無い/未使用の環境では黙ってスキップされます)。
...
sweepはrolloutを`task_complete`イベントの境界でターンに分割して復元するため、hook導入前のセッションもターン単位で再生成できます。通常実行は毎回履歴とcursorをresetするため、同じsourceから行が倍増しません。`--dry-run`は既存cursorを使わずsource先頭から読みますが、履歴・cursor・dashboard・設定を変更しません。
...
backupは作りません。元JSONLの消失・移動・破損行は復元できず、途中失敗時は部分履歴/cursorが残り得ます。その場合は同じ`sweep`を再実行してください。dashboardの日次更新stateはresetしますが、config・予算・通知・mute・単価表/為替cache・**Codexサブエージェント利用記録**は保持します。ただし、再生成した過去turnとCodexサブエージェント利用記録の表示用joinは失われる場合があります。サブエージェント料金は従来どおり未集計です。詳細は[sweep](sweep.md)を参照してください。
