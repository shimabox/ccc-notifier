...
- Codexでサブエージェントの利用を検出したターンには、**「利用あり(料金は独立レコードで計上)」**と開始・終了数が表示されます。これは親ターン上の利用検出表示で、サブエージェント分のトークン・料金そのものは**「メイン $0 + サブエージェント枠」の独立レコード**として別途履歴に入り、総額へはそちらで加算されます
