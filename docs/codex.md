...
npx ccc-notifier sweep --rebuild   # 履歴と取り込み位置を捨て、Claude/Codexの元JSONLから全再生成
```
...
通常sweepで取り込むUSDも、過去時点の単価を保存・再現せず、実行時に取得した単価表で計算する概算です。JPYも実行時の為替レートを使うため、当時の条件と一致する保証はありません。

`--rebuild`はClaude/Codexを終了・再起動せず実行できます。履歴・取り込み位置・自動生成dashboardをbackupなしで捨て、元JSONLから実行時点の単価・為替で全件を作り直します。dashboard HTMLは`dashboard.autoRegenerate=true`なら次の正常なターン、無効なら手動`dashboard` / `dashboard --all`で再生成します。rebuild中に完了したターンは通知が欠ける場合があり、通知の再送はありませんが、完了後は自動的に通常動作へ戻ります。元JSONLの消失・破損行は復元できず、途中失敗時は同じコマンドを再実行します。詳しいreset範囲と注意事項は[sweepの全再生成](sweep.md#履歴を捨てて全再生成--rebuild)を参照してください。

**Codexサブエージェント利用記録**の保存ファイルはrebuildでも残ります。ただしコスト履歴を作り直す際に過去turnとの表示用join keyは再構成しないため、過去turnに「利用あり」を表示する紐付けは失われる場合があります。サブエージェントの料金は従来どおり未集計です。
