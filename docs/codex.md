...
検査可能な標準user/project JSON sourceでccc-notifier所有のStop hookを確認できた場合、doctorは最新mtimeの**単一rollout**を先頭から読み、`Codex 最新rollout合計`を表示します。最新ファイルが親rolloutか子rolloutかは分類しないため、どちらか一方だけの値になる場合があります。これは常にAPI従量料金への参考換算で、Claude Code行とは別集計です。複数rollout、親/子rollout、Codexサブエージェント料金を合算せず、historyやcursorも更新しません。TOML、opaque、supplemental env-extraしか確認できない場合は、設定済みと推測して金額を表示しません。
...
- Codexでサブエージェントの利用を検出したターンには、**「利用あり(料金はこのターンに含めない)」**と開始・終了数が表示されます。これは親ターン上の利用検出表示で、子の料金は親ターンの金額に一切含まれません。集計対象の child rollout(fork 由来でないもの)のトークン・料金は**「メイン $0 + サブエージェント枠」の独立レコード**として別途履歴に入り、総額へはそちらで加算されます(fork 由来の child と未回収の過去分は、この表示があっても履歴にはまだ入っていません)
...
`sweep`は既存のコスト履歴と取り込み位置をresetし、Claude Code / Codexの手元に残る元JSONLから概算を再生成します。Codex対応後は`~/.codex/sessions`配下も自動的に走査対象になり、Codexサブエージェントのchild rolloutも**「メイン $0 + サブエージェント枠」の独立レコード**として履歴へ入ります(`~/.codex`が無い/未使用の環境では黙ってスキップされます)。

```bash
npx ccc-notifier sweep                  # Claude/Codexの全期間を再生成
npx ccc-notifier sweep --days 7         # reset後、直近7日だけを再生成
npx ccc-notifier sweep --dry-run        # 全期間の結果をsource先頭からpreview
...
sweepは通常rolloutを`task_complete`イベントの境界でターンに分割して復元するため、hook導入前のセッションもターン単位で再生成できます。進行中ターンのusageは直前の完了ターンへ混ぜず、そのターン自身のモデルとプロンプトへ分けます。通常実行は毎回履歴とcursorをresetするため、同じsourceから行が倍増しません。`--dry-run`は既存cursorを使わずsource先頭から読みますが、履歴・cursor・dashboard・設定を変更しません。

USDは過去時点の単価を保存・再現せず、sweep実行時に取得した単価表で計算する概算です。JPYも実行時の為替レートを使うため、以前の表示額や当時の条件と一致する保証はありません。

sweepはClaude/Codexを終了・再起動せず実行できます。進行中sourceもbest-effortで読みますが、同時truncate/rewriteは保証せず、読み取り後の末尾は後続hookが回収します。sweep中に完了したturnは通知が欠ける場合があり、通知の再送はありません。完了後は自動的に通常動作へ戻ります。

backupは作りません。元JSONLの消失・移動・破損行は復元できず、途中失敗時は部分履歴/cursorが残り得ます。その場合は同じ`sweep`を再実行してください。dashboardの日次更新stateはresetしますが、config・予算・通知・mute・単価表/為替cache・**Codexサブエージェント利用記録**は保持します。ただし、再生成した過去turnとCodexサブエージェント利用記録の表示用joinは失われる場合があります。サブエージェント分(child rollout)は独立レコードとして再生成されます。詳細は[sweep](sweep.md)を参照してください。
...
- **サブエージェント/collab スレッド(child rollout)の usage・料金は「メイン $0 + サブエージェント枠」の独立レコードとして集計します**。親のどのターンに属すかの対応付けは推測になるため行いません(親ターン上は4hookによる利用検出の表示のみ)。サブエージェント分は履歴・`report`・ダッシュボードの総額と月予算に含まれますが、この独立レコードからターン単位のOS/Slack通知は送りません
- **fork 由来の child rollout(session_meta に `forked_from_id` を持つもの)は集計対象外です**。fork は親スレッドの履歴と累積トークンカウンタを引き継ぐため、そのまま集計すると親の使用量を丸ごと二重計上します。fork 後の増分だけを切り出す基準点は rollout 内から機械的に判定できないため、安全側(計上しない)に倒しています
- **過去のサブエージェント分の回収**: 健全な既存カーソルが残っている通常のアップグレードでは自動回収されません。`reset-cursors`、`sweep`、またはカーソル消失・破損後の再走査で過去分が取り込まれます。**その際は過去のサブエージェント利用がまとめて入るため、履歴総額・月予算の消化が大きく増えることがあります**
