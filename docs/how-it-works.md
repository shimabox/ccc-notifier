...
ccc-notifierは**手元に残るログのトークン数**だけを集計します。ログにトークン数が書かれない使い方は、実装の未対応ではなく**データが存在しないため計測できません**(将来のバージョンでも同じです)。
...
| Claude デスクトップアプリの**ホームタブ通常チャット** | ❌ **取り込めない** | **トークン数がローカルのどこにも残らないため原理的に不可**。アプリのデータ領域を実測してもusage/token数を持つファイルは作られない |
| Claude の**Web版(claude.ai)チャット** | ❌ **取り込めない** | 同上(手元にログが無い) |
...
| Codexの**クラウド実行**(Web / GitHub / Slack / クラウド委任) | ❌ **取り込めない** | 実行が手元で行われず、rolloutが残らない |
| **ChatGPT通常チャット** | ❌ **取り込めない** | 同上(手元にログが無い) |

プラン(サブスクリプション)の使用率は、Claudeデスクトップアプリが`plan-usage-history.json`に持っていますが、これは**使用率%であってトークン数でも金額でもありません**。ccc-notifierは扱いません。
