...
- 手動 `dashboard` の引数なしは、`dashboard.days`(既定30日)分を `report.html` に生成します。全履歴が必要なときだけ `dashboard --all` で `report-all.html` を更新します。どちらもコスト推移グラフを **日 / 週 / 月** に切り替えられます。縦軸は **[金額] / [トークン]** も切り替えられ、トークン軸では棒が「キャッシュ(read+write)/ 入力(キャッシュ以外)/ 出力」の3区分積み上げになります(モデル別トークンは記録していないため、モデル別色分けは金額軸のみ)。
...
直近版の期間表示は、設定値だけでなく実際に埋め込まれた履歴を基準にします。例えば`dashboard.days=30`でも、最古から最新までがローカル暦日で4日分しかなければ「履歴 4 日分」と表示します。履歴のspanが設定期間を満たす場合は「直近 30 日版」です。これは生成時点の静的表示なので、日付が変わるだけではHTMLは変わりません。次の正常なターン、または手動`dashboard`で再生成したときに追従します。
...
チャートの縦軸を「トークン」に切り替えると、キャッシュ / 入力 / 出力の3区分でトークン使用量の推移が見えます:

![トークン推移(金額/トークン軸トグル)](images/dashboard-tokens.png)
...
サマリー・日別チャート・モデル別/プロジェクト別・合計はいずれも**サブエージェント分を含む総額**で表示します。サブエージェントを使ったターンがあると、ヒーローの合計の下に「うちサブエージェント $X(¥Y)」が表示され、モデル別内訳にはサブエージェントが使ったモデル(Sonnet 5 など)も並びます。ターン履歴では該当ターンのモデル欄に `+SA` が付き、行を展開すると「サブエージェント: $X(¥Y)· モデル名 · APIコール N · 計 X tokens(cache Y%)」の内訳が出ます(サブエージェントの完了タイミングにより、その分が次ターン以降の日に計上されることがあります)。

**トークン使用量の表示**: 金額が主役のまま、トークンを従属メトリクスとして各所に併記します。

- ヒーローの合計の下と KPI カード(今日・今週・今月・通算)に、期間の `in X(cache Y%)/ out Z tokens` を表示します(金額と同じく main + sidechain + サブエージェント合算。ソースフィルタにも連動)
- モデル別・プロジェクト別・サーフェス別の各テーブルに `tokens` 列を表示します。プロジェクト別・サーフェス別は正確な合算、モデル別はターン列と同じ参加カウント方式(登場ターンの全量を各モデルへ計上・重複あり)です
- **単価不明で金額 $0 になる利用分**(Codex Desktop の一部や Codex サブエージェントの guardian 等)も、tokens 列で利用規模が分かります

CLI 以外の利用(デスクトップアプリ等)があると「サーフェス別内訳」セクションが表示され、利用元(cli / desktop / vscode 等)ごとのターン数・金額・トークンが確認できます。
...
正常な`sweep`完了時は例外的に、`dashboard.autoRegenerate=true`なら日次状態にかかわらず両ファイルをその場で再生成し、相互リンクをすぐ利用できる状態にします。`dashboard.autoRegenerate=false`なら生成せず、手動`dashboard` / `dashboard --all`で更新します。`sweep --dry-run`と、sourceの一部が失敗したpartial failureでは生成しません。
...
履歴ファイルは両版で共有して1回だけ全件を読み込み・解析してから配列を期間で絞るため、読み込み処理自体は履歴総量に比例します(期間限定版でも、保存済み全履歴の当月分を集計対象から落とさないためです)。生成 HTML の自動リロード(既定30秒)を跨いでも、**選択中の粒度・選択期間・検索語・スクロール位置は保持**されます。
...
月予算カードは `report.html` の**期間限定版では、保存済み全履歴の当月分を集計対象から落とさず、その値に固定**され、グラフの期間選択には連動しません。`report-all.html` の**全履歴版では選択した暦月に連動**します。どちらも Claude Code / Codex CLI の全ソース合算です。
