...
- 手動 `dashboard` の引数なしは、全履歴を `report-all.html` に生成します。コスト推移グラフは **日 / 週 / 月** をボタンで切り替えられ、横スクロールで過去まで遡れます。
...
```bash
npx ccc-notifier dashboard            # 全履歴を report-all.html に生成して開く
...
引数なしでは**全履歴**を `report-all.html` に埋め込み、粒度(日/週/月)や期間の選択はブラウザ側で行います。`--days N` では直近N日だけを `report.html` に生成します。不正な `--days` は全履歴扱いです。`--out X` を指定した場合は X だけを生成し、canonical 2ファイルや日次状態には触れません。
...
- `report-all.html`: 全履歴を埋め込み、**ローカル日の最初の正常な新規ターンだけ**更新します。手動 `dashboard` でも直ちに更新できます。
...
相手側のcanonical HTMLがまだ未生成の場合、そのパスには生成方法と戻るリンクを載せたplaceholderを作ります。実体を生成した時点でplaceholderはatomicに上書きされます。
...
手動 `dashboard` の既定では全履歴(プロンプト全文を含む)を埋め込むため、履歴が増えるとファイルが大きくなります。プライバシー上プロンプトを残したくない場合や、古い履歴を消したい場合は `history` コマンドを使います。**元に戻せない操作**なので、`--yes` を付けなければ対象件数を表示して確認します。
