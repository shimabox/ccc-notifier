...
`dashboard` コマンドは、蓄積した履歴を1枚の HTML(既定 `~/.ccc-notifier/report.html`)に書き出してブラウザで開きます。履歴(`history.jsonl`)と `config.json` だけを読むため、通知をすべて無効にした[通知なしモード](configuration.md#通知なしモード記録ダッシュボードのみ--dashboard-only-mode)でもそのまま動作します。サマリーカード(今日 / 今週(直近7日)/ 今月(暦月)/ 通算)、コスト推移のモデル別積み上げ棒グラフ、モデル別・プロジェクト別の内訳、検索・行展開できるターン履歴を表示します。生成物は CSS/JS/SVG をすべてインライン化した**完全自己完結・オフライン動作・外部通信ゼロ**のファイルで、OS のライト/ダーク設定に追従します。プロンプト全文はブラウザで実行されない形(JSON 埋め込み + textContent 描画)で安全に展開されます。
...
- 手動実行の既定では全履歴を埋め込むため、プロンプトが多いとファイルが大きくなります。気になる場合は後述の **`history redact` / `history clear`** で履歴を整理できます。

```bash
npx ccc-notifier dashboard            # 既定は全履歴・生成してブラウザで開く
...
Claude Code の応答が完了するたび(Stop hook の `track` 実行時)に、この `report.html` を**自動で再生成**します。自動再生成では `config.json` の `dashboard.days`(既定30日)分だけを埋め込み、HTML構築・ファイル書き込み・ブラウザでの読み込みと描画の負荷を抑えます。一方、履歴ファイルは1回だけ全件を読み込み・解析してから配列を期間で絞るため、この処理は履歴総量に比例します(当月予算を埋め込み対象外も含めて正確に集計するためです)。この範囲は手動 `dashboard` の既定(全履歴)には影響しません。さらに生成される HTML には**自動リロード**(既定30秒ごと)が仕込まれているため、`report.html` をブラウザのタブで開きっぱなしにしておくと、Claude Code を使っている間そのタブが**約30秒ごとに最新の内容へ更新**されていきます(最新化のタイミングは Claude Code の応答完了時)。自動リロードを跨いでも、**選択中の粒度(日/週/月)・選択期間・検索語・スクロール位置は保持**されます。保持された期間が新しい埋め込み範囲に存在しない場合は、空表示を避けるため対象期間合計へ戻ります。
...
月予算カードは、自動再生成などの**期間限定版では全履歴から集計した当月の値に固定**され、グラフの期間選択には連動しません。手動 `dashboard` の既定で生成する**全履歴版では従来どおり選択した暦月に連動**します。どちらも Claude Code / Codex CLI の全ソース合算です。
