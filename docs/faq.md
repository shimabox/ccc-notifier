...
- Codexのサブエージェント(child rollout)のtoken/料金は、**メイン金額 `$0`・プロンプトなしの独立した履歴**(サブエージェント枠)として集計します。ただし fork 由来の child(親スレッドの履歴と累積カウンタを引き継ぐ形式)は、親の使用量を二重計上しないため集計対象外です。親のどのturnに属すかの対応付けは推測になるため行いません。親turn上の「利用あり(料金は独立レコードで計上)」表示は4hook(Stop / UserPromptSubmit / SubagentStart / SubagentStop)による利用検出で、すべてが信頼・有効である必要があります。一部が動かない、または関連先が曖昧な場合は、誤ったturnへ付けず表示を省略します。UserPromptSubmitに含まれるprompt本文は保存しません
