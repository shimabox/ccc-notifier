...
- Codexのサブエージェントはchild token/料金を集計せず、Historyに「利用あり・料金未集計」と表示するための利用記録だけです。正しい親turnへ結び付けるには Stop / UserPromptSubmit / SubagentStart / SubagentStop の4hookがすべて信頼・有効である必要があります。一部が動かない、または関連先が曖昧な場合は、誤ったturnへ付けず表示を省略します。UserPromptSubmitに含まれるprompt本文は保存しません
