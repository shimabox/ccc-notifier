...
- サブエージェントが親の応答より後に完了した場合は、同じセッションの次のStop hook、または`sweep`で回収します。別の親ターンには付けず、**メイン金額 `$0`・プロンプトなし**の独立した履歴として表示します。これは取りこぼしではなく、サブエージェント分だけを正しい完了時刻で残すための表示です
- この独立した履歴からOS/Slack通知は送りません。サブエージェントの金額は通常どおり履歴・`report`・ダッシュボードの総額に含まれます
